...
const CHAPTERS = [
  ...
  { id: 'avoiding-food-waste-contamination',   number: '03', label: 'Avoiding Food Waste Contamination',   emoji: '🚫' },
];
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.chapterList}>
          {CHAPTERS.map((ch) => (
            <TouchableOpacity key={ch.id} style={styles.chapterRow} activeOpacity={0.75}
              onPress={() => router.push(`/(tabs)/FoodOrganicWasteChapters?chapter=${ch.id}` as any)}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.startBtn} onPress={() => router.push(`/(tabs)/FoodOrganicWasteChapters?chapter=${CHAPTERS[0].id}` as any)} activeOpacity={0.85}>
