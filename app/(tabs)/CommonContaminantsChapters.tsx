...
const CHAPTER_DATA: Record<string, ChapterData> = {
  'coffee-cups-takeaway-packaging': {
    id: 'coffee-cups-takeaway-packaging',
    navTitle: 'Coffee Cups & Takeaway Packaging',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn why coffee cups and takeaway packaging are hard to recycle.',
    parts: [
      { number: '01', label: "Why Coffee Cups Aren't Recyclable", emoji: '☕' },
      { number: '02', label: 'Plastic-Lined Packaging',           emoji: '📦' },
      { number: '03', label: 'Correct Disposal',                  emoji: '🗑️' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
  'greasy-food-soiled-items': {
    id: 'greasy-food-soiled-items',
    navTitle: 'Greasy & Food-Soiled Items',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn how food residue contaminates the recycling stream.',
    parts: [
      { number: '01', label: 'Pizza Boxes!!',  emoji: '🍕' },
      { number: '02', label: 'Greasy Paper',   emoji: '🧻' },
      { number: '03', label: 'Food Residue',   emoji: '🍔' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
  'problem-plastics': {
    id: 'problem-plastics',
    navTitle: 'Problem Plastics',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn which plastics cause problems in the recycling process.',
    parts: [
      { number: '01', label: 'Plastic Bags',  emoji: '🛍️' },
      { number: '02', label: 'Film & Wrap',   emoji: '🎁' },
      { number: '03', label: 'Black Plastic', emoji: '🖤' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
};
...
export default function CommonContaminantsChapters() {
  const { chapter } = useLocalSearchParams<{ chapter: string }>();
  const data = CHAPTER_DATA[chapter ?? ''] ?? CHAPTER_DATA['coffee-cups-takeaway-packaging'];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.nav}>
        <TouchableOpacity onPress={() => router.push('/(tabs)/CommonContaminants' as any)} style={styles.backBtn}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        <View style={styles.heroBox}>
          <View style={styles.heroPlaceholder}>
            <Text style={styles.heroEmoji}>{data.id === 'coffee-cups-takeaway-packaging' ? '☕' : data.id === 'greasy-food-soiled-items' ? '🍕' : '🛍️'}</Text>
          </View>
        </View>
