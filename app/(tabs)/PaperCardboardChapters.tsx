...
const CHAPTER_DATA: Record<string, ChapterData> = {
  'paper-recycling-basics': {
    id: 'paper-recycling-basics',
    navTitle: 'Paper Recycling Basics',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn how paper is sorted, cleaned, and recycled.',
    parts: [
      { number: '01', label: 'Clean vs. Dirty Paper',       emoji: '📄' },
      { number: '02', label: 'Magazines & Newspapers',      emoji: '🗞️' },
      { number: '03', label: 'Envelopes & Windows',         emoji: '✉️' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
  'cardboard-rules': {
    id: 'cardboard-rules',
    navTitle: 'Cardboard Rules',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn the rules for recycling cardboard correctly.',
    parts: [
      { number: '01', label: 'Greasy Pizza Boxes',  emoji: '🍕' },
      { number: '02', label: 'Flattening Boxes',    emoji: '📦' },
      { number: '03', label: 'Removing Tape',       emoji: '📏' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
  'composite-paper-packaging': {
    id: 'composite-paper-packaging',
    navTitle: 'Composite Paper Packaging',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn about composite packaging and how to handle it.',
    parts: [
      { number: '01', label: 'Juice Cartons',    emoji: '🧃' },
      { number: '02', label: 'Laminated Paper',  emoji: '🗂️' },
      { number: '03', label: 'Coffee Cups',      emoji: '☕' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
};
...
export default function PaperCardboardChapters() {
  const { chapter } = useLocalSearchParams<{ chapter: string }>();
  const data = CHAPTER_DATA[chapter ?? ''] ?? CHAPTER_DATA['paper-recycling-basics'];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.nav}>
        <TouchableOpacity onPress={() => router.push('/(tabs)/PaperCardboard' as any)} style={styles.backBtn}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        <View style={styles.heroBox}>
          <View style={styles.heroPlaceholder}>
            <Text style={styles.heroEmoji}>{data.id === 'paper-recycling-basics' ? '📄' : data.id === 'cardboard-rules' ? '📦' : '🧃'}</Text>
          </View>
        </View>
