...
const CHAPTER_DATA: Record<string, ChapterData> = {
  'bin-colours-meanings': {
    id: 'bin-colours-meanings',
    navTitle: 'Bin Colours & Meanings',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn what each bin colour means in your area.',
    parts: [
      { number: '01', label: 'Recycling Bin',     emoji: '♻️' },
      { number: '02', label: 'General Waste Bin', emoji: '🗑️' },
      { number: '03', label: 'Food Waste Bin',    emoji: '🥦' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
  'accepted-items-by-postcode': {
    id: 'accepted-items-by-postcode',
    navTitle: 'Accepted Items by Postcode',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Learn what your local area accepts for recycling.',
    parts: [
      { number: '01', label: 'What Your Area Accepts', emoji: '✅' },
      { number: '02', label: 'What Your Area Rejects', emoji: '❌' },
      { number: '03', label: 'Seasonal Changes',       emoji: '📅' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
  'collection-service-updates': {
    id: 'collection-service-updates',
    navTitle: 'Collection & Service Updates',
    title: 'Understanding Paper Recycling Basics',
    subtitle: 'Stay up to date with your local collection service.',
    parts: [
      { number: '01', label: 'Collection Days',    emoji: '📅' },
      { number: '02', label: 'Missed Collections', emoji: '🚛' },
      { number: '03', label: 'Service Disruptions',emoji: '⚠️' },
    ],
    quizzes: ['Part 01 Quiz', 'Part 02 Quiz', 'Part 03 Quiz'],
  },
};
...
export default function LocalRulesChapters() {
  const { chapter } = useLocalSearchParams<{ chapter: string }>();
  const data = CHAPTER_DATA[chapter ?? ''] ?? CHAPTER_DATA['bin-colours-meanings'];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.nav}>
        <TouchableOpacity onPress={() => router.push('/(tabs)/LocalRules' as any)} style={styles.backBtn}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        <View style={styles.heroBox}>
          <View style={styles.heroPlaceholder}>
            <Text style={styles.heroEmoji}>{data.id === 'bin-colours-meanings' ? '🗑️' : data.id === 'accepted-items-by-postcode' ? '📮' : '🚛'}</Text>
          </View>
        </View>
