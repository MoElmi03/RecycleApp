...
// Map of emoji to a background colour pair for the card
const EMOJI_COLORS: Record<string, string> = {
  ...
  '🌡️':'#FDECEA','🌱':'#D4EDDA','🌻':'#FFF9C4','♻':'#D4EDDA','🛒':'#F3E5F5',
  '🫕':'#FFF9C4','🔍':'#E8F4FD','🧪':'#E8F4FD','📊':'#E8F4FD','🐚':'#E8F4FD',
  '🌸':'#F3E5F5','🧻':'#F3E5F5','🍔':'#FFF9C4','☀️':'#FFF9C4','🔎':'#E8F4FD',
};
...
export default function LessonPlayer() {
  ...
  function goNext() {
    if (slideIndex < total - 1) {
      animateCard(() => setSlideIndex(i => i + 1));
    }
  }
  function goPrev() {
    if (slideIndex > 0) {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.completionBg}>
          <Animated.View style={[styles.completionCard, { opacity: rewardAnim, transform: [{ scale: rewardScale }] }]}>
            ...
            <View style={styles.completionActions}>
              ...
              <TouchableOpacity
                ...
                onPress={() => router.push(`/(tabs)/QuizPlayer?id=${id}&returnTo=${returnTo ?? ''}` as any)}
              >
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerChapter}>{lesson.chapterTitle}</Text>
          <Text style={styles.headerPart}>{lesson.partTitle}</Text>
        </View>
        <View style={{ width: 38 }} />
      </View>
      ...
      <View style={styles.navRow}>
        ...
        >
          <Feather name="chevron-left" size={22} color={slideIndex === 0 ? '#ccc' : '#606C38'} />
...
const styles = StyleSheet.create({
  ...
  headerChapter:     { fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.8 },
  headerPart:        { fontSize: 17, color: '#fff', fontWeight: '700', marginTop: 2 },
  ...
  slideCard:         { borderRadius: 24, padding: 28, minHeight: 380, justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 16, elevation: 4 },
  slideEmoji:        { fontSize: 64, textAlign: 'center', marginBottom: 20 },
  slideTitle:        { fontSize: 22, fontWeight: '800', color: '#1A1A1A', textAlign: 'center', marginBottom: 16, lineHeight: 30 },
  slideBody:         { fontSize: 16, color: '#333', lineHeight: 26, textAlign: 'center' },
  ...
  navBtnText:        { fontSize: 15, color: '#606C38', fontWeight: '600', marginLeft: 4 },
  ...
  finishBtn:         { backgroundColor: '#FEFAE0', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 14 },
  finishBtnText:     { fontSize: 16, color: '#283618', fontWeight: '800' },
