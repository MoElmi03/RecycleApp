...
const CHAPTERS = [
  { id: 'coffee-cups-takeaway-packaging', number: '01', label: 'Coffee Cups & Takeaway Packaging', emoji: '☕' },
  { id: 'greasy-food-soiled-items',       number: '02', label: 'Greasy & Food-Soiled Items',       emoji: '🍕' },
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.chapterList}>
          {CHAPTERS.map((ch) => (
            <TouchableOpacity key={ch.id} style={styles.chapterRow} activeOpacity={0.75}
              onPress={() => router.push(`/(tabs)/CommonContaminantsChapters?chapter=${ch.id}` as any)}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.startBtn} onPress={() => router.push(`/(tabs)/CommonContaminantsChapters?chapter=${CHAPTERS[0].id}` as any)} activeOpacity={0.85}>
